...
contract DIGIVERSE is Context, IERC20, Ownable {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (toLP && NOTfromExcluded) {
            ...
            if (sellFee > 0) liqFee = (liquidityFromFees * fee) / sellFee;
        } else if (fromLP && NOTtoExcluded) {
            fee = (buyFee * amount) / 100;
            if (buyFee > 0) liqFee = (liquidityFromFees * fee) / buyFee;
        } else if (NOTtoExcluded && NOTfromExcluded) {
            fee = (transferFee * amount) / 100;
            if (transferFee > 0)
                liqFee = (liquidityFromFees * fee) / transferFee;
        }
        ...
        if (fee > 0) {
            ...
            fee -= liqFee;
            liquidityTokensCollected += liqFee;
    ...
    function swapAndLiquify() public lockTheSwap {
        ...
        if (totalTokens >= liquidityTokensCollected) {
            totalTokens -= liquidityTokensCollected;
            uint toSwapForLiquidity = liquidityTokensCollected / 2;
            liquidityTokensCollected -= toSwapForLiquidity;
            swapTokensForEth(toSwapForLiquidity);
            ...
            (, , newBalance) = uniswapV2Router.addLiquidityETH{
                value: newBalance
            }(
                address(this),
                toSwapForLiquidity,
            ...
            totalLiquidityCollected += newBalance;
